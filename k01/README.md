...
## ソースコードの説明
関数LinearSearchの実装
  探すデータのid、データ配列、データ総数を受取り、先頭から順に探索を行う。

関数BinarySearchの実装
  探すデータのid、データ配列、探索に使用するインデックス（left,right）を受け取り、バイナリサーチを行う。
  leftとrightのちょうど中間のインデックスを変数iとして保持し、array[i]のidとkeyの比較を行う。keyのほうが大きい場合leftをi+1の位置に置き換え、keyのほうが小さい場合rightをi-1の位置に置き換える処理をデータが一致するか探索が最後まで行われるまで実行する。
  今回配列に格納されているidは順に並んでいるためソート処理は必要ない。
